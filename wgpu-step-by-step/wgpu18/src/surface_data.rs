...
impl ParametricSurface {
    pub fn new(ps: ParametricSurface) -> (Vec<[f32;3]>, Vec<[f32;3]>, Vec<[f32;3]>, Vec<u32>) {
        ...
        let (min, max) = parametric_surface_range(ps.f, ps.umin, ps.umax, ps.vmin, ps.vmax, 
            ps.u_segments, ps.v_segments, ps.scale, ps.aspect, ps.params, cd);
        ...
        for i in 0..=ps.u_segments {
            ...
            for j in 0..= ps.v_segments {
                ...
                p0 = Vector3::new(pa[0]*ps.scale, ps.scale*ps.aspect*pa[1], ps.scale*pa[2]);

                // calculate normals
                if u-eps >= 0.0 {
                    pa = (ps.f)(u-eps, v, ps.params);
                    p1 = Vector3::new(pa[0]*ps.scale, ps.scale*ps.aspect*pa[1], ps.scale*pa[2]);
                    p2 = p0 - p1;
                } else {
                    pa = (ps.f)(u+eps, v, ps.params);
                    p1 = Vector3::new(pa[0]*ps.scale, ps.scale*ps.aspect*pa[1], ps.scale*pa[2]);
...
fn parametric_surface_range(f: fn(f32, f32, [f32; 5]) -> [f32; 3], umin:f32, umax:f32, vmin:f32, vmax:f32, 
  nu:usize, nv: usize, scale:f32, aspect:f32, params:[f32; 5], dir:usize) -> (f32, f32) {
